...
public abstract class Player extends GameObject {
    ...
    protected Key PUNCH_KEY = Key.Q; // Punch attack key for Player 1
    protected Key KICK_KEY = Key.R; // Kick attack key for Player 1
    protected Key DASH_KEY = Key.F; // Dash key
    ...
    public void update() {
        ...
        if (levelState == LevelState.RUNNING) {
            ...
            if (attacksEnabled && Keyboard.isKeyDown(ATTACK_KEY) && !keyLocker.isKeyLocked(ATTACK_KEY)) {
                ...
                float speed = ProjectileAttack.getPlayer1ProjectileSpeed();
            ...
            // punch attack input: create a punch hitbox when punch key is pressed
            if (attacksEnabled && Keyboard.isKeyDown(PUNCH_KEY) && !keyLocker.isKeyLocked(PUNCH_KEY)) {
                ...
                try {
                    ...
                    if (Engine.Debug.ENABLED) System.out.println("DEBUG: Player 1 punch attack at x=" + punchX + " y=" + punchY);
                } catch (Exception e) {
                    if (Engine.Debug.ENABLED) System.out.println("DEBUG: Failed to spawn player punch attack: " + e);
                }
            }

            // kick attack input: create a kick hitbox when kick key is pressed
            if (attacksEnabled && Keyboard.isKeyDown(KICK_KEY) && !keyLocker.isKeyLocked(KICK_KEY)) {
                ...
                try {
                    ...
                    if (Engine.Debug.ENABLED) System.out.println("DEBUG: Player 1 kick attack at x=" + kickX + " y=" + kickY);
                } catch (Exception e) {
                    if (Engine.Debug.ENABLED) System.out.println("DEBUG: Failed to spawn player kick attack: " + e);
                }
            }
    ...
    public void draw(GraphicsHandler graphicsHandler) {
        ...
        // Draw current projectile name above Player 1 (use Player 1's projectile type)
